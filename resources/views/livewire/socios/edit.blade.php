        ...
        <div class="card-body">
            <div class="row">
                <div class="form-group col-6">
                    <label>Departamento
                        <code>*</code></label>
                    <select class="custom-select form-control-border border-width-2 @error('selectedDepartamento')
                        is-invalid @enderror" wire:model="selectedDepartamento">
                        @if (!$maquinaria->conductor)
                        <option selected>Seleccionar...</option>
                        @foreach($departamentos as $departamento)
                        <option value="{{ $departamento->ID_DEPARTAMENTO }}">{{ $departamento->NO_DEPARTAMENTO }}
                        </option>
                        @endforeach
                        @else
                        <option selected>Seleccionar...</option>
                        @foreach($departamentos as $departamento)
                        <option value="{{ $departamento->ID_DEPARTAMENTO }}" @if($maquinaria->conductor->id === $conductor->id)
                            selected='selected'
                            @endif>{{ $departamento->NO_DEPARTAMENTO }}
                        </option>
                        @endforeach
                        @else
                    </select>

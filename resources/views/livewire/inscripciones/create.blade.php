<div class="container">
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Registrar una inscripción</h3>
        </div>
        <div class="card-body">
            <div class="row">
            ...
            <div class="row">
                <div class="form-group col">
                    <label>Persona
                        <code>*</code></label>
                    <select
                        class="custom-select form-control-border border-width-2 @error('FK_SOLICITADO') is-invalid @enderror"
                        wire:model="FK_SOLICITADO">
                        <option selected>Seleccionar...</option>
                        @foreach ($personas as $persona)
                        <option value="{{$persona->ID_PERSONA}}">{{$persona->NO_SOCIO}}</option>
                        @endforeach
                    </select>
                    @error('FK_SOLICITADO')
                    <small class="form-text text-danger">{{$message}}</small>
